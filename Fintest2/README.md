...
> 嗨，歡迎參加 OOP 期中上機考，本題目為不算分之題目，目的僅讓你測試是否可推送至系統。
>
> 祝考生們能順利推送並且通過本次期末上機考， GL & HF !

## 題目敘述

在這個任務中，你會擁有一個你自己的功課版本庫（Repository），他會叫做 `midtest`，接著你就能根據以下的任務開始逐一完成。
...
```bash=
git clone git@gitea.is1ab.com:<學號>/Fintest.git
git clone https://gitea.is1ab.com/<學號>/Fintest.git
```
兩者使用其中一個即可，你可以於該 `Question` 頁面的右下角找到該版本庫下載連結。
透過這個指令後，你就能夠將遠端專案庫下載至本地端，並且它已經預先幫你設置好了遠端版本庫為 `https://gitea.is1ab.com/<學號>/midtest.git`。
...
請在 `src/hello.cpp` 中，將第 `4` 行程式碼註解 `//` 移除。  
並且確認可以通過編譯。
...
```
git remote set-url origin git@gitea.is1ab.com:<學號>/Fintest.git
// 這行不需要 (You don't need this command)
```
